...
import { getFuturesCardStorage, setFuturesCardStorage, getSpotCardStorage, setSpotCardStorage } from '../utils/localStorageUtils';
import { setSpotFavoriteCoins, setFuturesFavoriteCoins } from '../utils/reduxStorage';

const MarketPricesCard = ({ isSpot = false }) => {
  const localStorageData = isSpot ? getSpotCardStorage() : getFuturesCardStorage();
  const [selectedTab, setSelectedTab] = useState(localStorageData?.selectedTab || 'all');
  const [sortOrder, setSortOrder] = useState('default');
  const [searchedCoins, setSearchedCoins] = useState([]);
  const { spotCoinData, spotCoinList, spotFavoriteCoins, futuresCoinData, futuresCoinList, futuresFavoriteCoins } = useSelector((state) => state.dataStore);
  ...
  const sortedAllCoins = () => {
    ...
    if (searchedCoins.length > 0) {
      ...
      coins = selectedFavoriteCoins.map(symbol => {
        return selectedCoinData.find(data => data.symbol === symbol) || null;
      }).filter(coin => coin !== null);
    }
    else {
  ...
  const handleSearch = (result) => {
    ...
    if (result) {
			const filteredResults = selectedCoinList ? selectedCoinList?.filter(item =>
